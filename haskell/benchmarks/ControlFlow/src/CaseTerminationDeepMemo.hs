...
type MemoryConc = KeyValueArray Bool [Var Int]
...
analyze :: Var CFG -> StateConc [Var Int]
analyze cfg  = --trace "analyze" $
               let _ns = _nodes' cfg
                   cases = filter' isCase _ns in retrieveOrRun True (\_ -> return $ map (fmap _nID) $ filter' (not' ^. (terminatedCase cfg)) cases) 
